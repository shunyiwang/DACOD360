...
def init_multi_users(user_num,opt): #初始化用户
	users = []
	for user_idx in range(user_num):
		users.append(env_user(user_idx,user_idx % RRH_MAX + 1,opt)) 
	return users
...
def update_request_queue(time_stamp,users,request_queue,last_request,response,cache_table,user_num):
	...
	response['qoe'], response['bitrate'], response['qoe_vector']= users[response['user_idx']].evaluate_qoe(response)
