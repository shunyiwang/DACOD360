...
class Environment:
    def __init__(self, bw_capacity, all_time, all_cooked_bw):
        ...
        self.backhaul_bw = []
        temp_bw  = self.cooked_bw[:66]
        #self.backhaul_bw = temp_bw
        
        
        
        for bw in self.cooked_bw:
            self.backhaul_bw.append(bw*bw_capacity/np.mean(temp_bw))
    ...
    def download_tile_time(self, time_stamp, tile_size):
        ...
        self.virtual_mahimahi_ptr = int(math.floor(time_stamp)) % len(self.backhaul_bw)                      
        ...
        while True:  # download video tile over mahimahi
            ...
            if self.virtual_mahimahi_ptr >= len(self.backhaul_bw):
