...
def rl_decision(strategy,variable,user_num,bw_capacity,bw_fluctuation,cache_max,trace_idx,zipf,i):            
    ...
    with tf.Session() as sess:
        ...
        while True:  # experience video streaming forever
            ...
            if strategy == 'DACOD360' or strategy == 'NO_CACHE':
                info_cluster = gamebased_allocating.gamebased_allocating(info_cluster,presupposed_bw)
            elif strategy == 'Offline_OPT':
                predict_backhaul_bandwidth = net_env.backhaul_bw[int(round(time_stamp,0))] * KB_IN_GB / BITS_IN_BYTE 
                presupposed_bw = ACTION[action]*predict_backhaul_bandwidth
                info_cluster = uniform_allocating.uniform_allocating(info_cluster,min(all_size),presupposed_bw)                 
            elif strategy == 'DRL_ONLY':
                info_cluster = uniform_allocating.uniform_allocating(info_cluster,min(all_size),presupposed_bw)
