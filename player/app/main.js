...
app.controller('DashController', ['$scope','$interval', function ($scope, $interval) {
    ...
    $scope.selectedItem = {  // Save the selected media source
        type:"json",
        value:"http://202.38.68.70:8880/A380/aframeVP907.json"
    };
    ...
    $scope.chartData_quality1 = [];  // Save the qualtiy data need to put on the charts
    $scope.chartData_buffer1 = [];  // Save the buffer data need to put on the charts
    $scope.chartData_throughput1 = [];  // Save the throughput data need to put on the charts
    $scope.chartState = {  // Save the charts' states
        quality:{
            ...
            video_6:{
                data: [], color: '#ff99cc', label: 'video_6'
            },
            video_7:{
                data: [], color: '#a0522d', label: 'video_7'
            },
            video_8:{
                data: [], color: '#ff00ff', label: 'video_8'
            },
            audio:{
    ...
    $scope.availableStreams = [  // [For setting up the media source] All the available preset media sources
        {
            name:"LVOD",
            json:"http://202.38.68.70:1880/A380/aframeVP907.json",
        },
        {
            name:"SVOD",
            json:"http://127.0.0.1:5500/CMPVP907/aframeVP907.json",
        },
        {
            name:"LIVE",
            json:"http://127.0.0.1:5500/CMPVP907/aframeVP907tile.json",
        },
        {
            name:"BUNNY",
        ...
        switch(type) {
            case "quality":
                ...
                var data = {
                    ...
                    yaxis: $scope.chartData_quality1.length + 1,
                    type: type
                };
                $scope.chartData_quality1.push(data);
            ...
            case "buffer":
                var data = {
                    ...
                    label: $scope.chartState[type][id].label,
                    color: $scope.chartState[type][id].color,
                    yaxis: $scope.chartData_buffer1.length + 1,
                    type: type
                };
                $scope.chartData_buffer1.push(data);
                $scope.chartOptions.yaxes.push({
                    axisLabel: data.label
                });
                break;
            case "throughput":
                var data = {
                    id: id,
                    data: $scope.chartState["throughput"][id].data,
                    label: $scope.chartState["throughput"][id].label,
                    color: $scope.chartState["throughput"][id].color,
                    yaxis: $scope.chartData_throughput1.length + 1,
                    type: type
                };
                $scope.chartData_throughput1.push(data);
                $scope.chartOptions.yaxes.push({
                    axisLabel: data.label
                });
                break;
        }
        $scope.chartOptions.legend.noColumns = Math.min($scope.chartData_quality1.length, 5);
    };
    ...
    function computeQoE() {
        ...
        for (let i = 0; i < $scope.playerCount; i++) {
            // Computing FOV score
            $scope.playerFOVScore[i] = 100 - 100 * (($scope.playerDivation[i] - Math.min.apply(Math,$scope.playerDivation)) / (Math.max.apply(Math,$scope.playerDivation) - Math.min.apply(Math,$scope.playerDivation)));
            if(i == 0){$scope.sumFOVScore = 0}
            $scope.sumFOVScore = $scope.sumFOVScore + $scope.playerFOVScore[i];
            ...
            if ($scope.ssresults) {
                ...
                if ($scope.ssresults[currentIndexString] != NaN && $scope.ssresults['maximum'] != NaN && $scope.ssresults['minimum'] != NaN) {
                    ...
                    if(i == 0){$scope.sumContentScore = 0}
                    $scope.sumContentScore = $scope.sumContentScore + $scope.playerContentScore[i];
                }
            }
            ...
            switch ($scope.qQOE) {
                case 'linear':
                    ...
                    previewerQOE = previewerQOE + MATH.max($scope.playerFOVScore[i]-50,0) * (($scope.playerBitrateList[i][$scope.playerDownloadingQuality[i]].bitrate - $scope.playerBitrateList[i][0].bitrate) - $scope.lambdaQOE * Math.abs($scope.playerBitrateList[i][$scope.playerDownloadingQuality[i]].bitrate - $scope.playerBitrateList[i][$scope.playerPastDownloadingQuality[i]].bitrate));
                    precontentQOE = precontentQOE + MATH.max($scope.playerFOVScore[i]-50,0) * $scope.playerContentScore[i] * (($scope.playerBitrateList[i][$scope.playerDownloadingQuality[i]].bitrate - $scope.playerBitrateList[i][0].bitrate) - $scope.lambdaQOE * Math.abs($scope.playerBitrateList[i][$scope.playerDownloadingQuality[i]].bitrate - $scope.playerBitrateList[i][$scope.playerPastDownloadingQuality[i]].bitrate));
                    break;
                case 'log': 
        ...
        $scope.totalQOE = pretotalQOE / $scope.playerCount - x// + $scope.normalizedTime - (now - $scope.startupTime) / 1000;
        $scope.viewerQOE = previewerQOE / $scope.playerCount- x * $scope.sumFOVScore/ ( $scope.playerCount)// + ($scope.normalizedTime - (now - $scope.startupTime) / 1000) * $scope.sumFOVScore/ ( $scope.playerCount);
        $scope.contentQOE = precontentQOE / $scope.playerCount- x * $scope.sumFOVScore * $scope.sumContentScore / ($scope.playerCount * $scope.playerCount)// + ($scope.normalizedTime - (now - $scope.startupTime) / 1000) * $scope.sumFOVScore * $scope.sumContentScore / ($scope.playerCount * $scope.playerCount);
        $scope.playerPastDownloadingQuality = $scope.playerDownloadingQuality;
        $scope.pausetime = x;
    }
