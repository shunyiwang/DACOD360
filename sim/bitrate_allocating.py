...
def species_origin(choromosome_length, new_request_queue):#初始化种群 生成chromosome_length大小的population_size个个体的种群
	population=[]
...
def calculate_fitness(population,choromosome_length,new_request_queue):# 目标函数相当于环境 对染色体进行筛选
	all_fitness = []
...
def predict_utility(chromosome, new_request_queue):#计算每条染色体的效用，即qoe的连乘积，如果错过deadline则为0
	utility = 1.0
	size = 0.0
...
def crossover(population, new_request_queue):#交叉操作,pc是概率阈值,采用单点交叉
...
def mutation(population, new_request_queue):     # pm是概率阈值
